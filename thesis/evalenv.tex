...
Bound variables are looked up in the environment; this lookup may fail if the variable does not exist in the environment, in which case the variable is left unchanged. For other primary expressions, the expression is left unchanged. When a hole is encountered, it is wrapped using the closure environment. A closure will never exist outside the evaluation boundary in the evaluation result.
...
\subsubsection{Additional constraints due to hole closure numbering}
