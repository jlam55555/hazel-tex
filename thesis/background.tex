...
In particular, the work performed for this thesis focuses on rewriting the dynamic semantics of Hazel to use an environment-based evaluation rather than a substitution-based evaluation. Hazel's internal language and evaluation semantics heavily borrow from the gradually-typed $\lambda$-calculus, which are incrementally introduced in this section. Introductory programming language material is typically presented using a substitution-based semantics, and we reproduce this here. Much of this material is condensed from introductory programming-language texts such as \cite{harper2016practical}.

After describing programming language specifications, an overview of programming language implementations is provided. Lastly, we present related structural editors.
...
Church introduced the \textit{untyped $\lambda$-calculus} $\Lambda$ as an example of a simple universal language of computable functions, and it forms the foundation for the syntax and evaluation semantics of functional programming languages.

The grammar of $\Lambda$ is very simple, only comprising three forms (excluding parentheses\footnote{The imperative programmer with a background in a C-family language be warned: parentheses are not required for function application. Rather, space (\textvisiblespace) is an infix operator that represents function application in $\Lambda$ and many functional languages. It traditionally is left-associative and has the highest precedence of any infix operator. Parentheses around function arguments are only required when it affects the order of operations. An exception to this rule in functional programming is in the LISP family of languages, in which parentheses specify function application rather than operator precedence, but that is not the interpretation here.}), shown below.
...
The static semantics of this syntax are very simple: every expression in $\Lambda$ is well-fromed if all variables are bound by some binder\footnote{There are no typing rules in the static semantics, because there is only a single type: the recursive arrow type $\tau::=\tau\to\tau$. Thus, it may be more correct to say that $\Lambda$ is ``uni-typed'' as opposed to ``untyped,'' as noted in \cite{harper2016practical}. Thus no type errors will occur when evaluating a (well-formed) expression in $\Lambda$.}.
...
\begin{singlespace}
  \begin{mathpar}
    \Infer{$\Lambda$-ELam}{}{\lambda x.e\Downarrow\lambda x.e}
    \and
    \Infer{$\Lambda$-EAp}{
...
$\Lambda$ is an example of a Turing-complete language. One of the key characteristics to this is the ability to compute recursive algorithms. To implement recursion, a function must be able to refer to itself. Since there is no construct to bind an expression to a variables other than function binders (i.e., there is no construct such as OCaml's \mintinline{ocaml}|let rec| expressions), one must pass a self-reference of a function to itself. For example, let us consider the example of a factorial function in $\Lambda$ (for sake of illustration, extended with a conditional statement, integers, and simple integer operators).
...
A more thorough discussion of $\Lambda$ and the Y-combinator is left to standard material on programming language theory, such as \cite{harper2016practical}.
...
The \textit{simply-typed $\lambda$-calculus} (STLC) $\Lambda^\to$ extends $\Lambda$ with one or more base types $b_i$, such as integers, booleans, or floating-point numbers. Consider the case of a single base type $b$. The extended grammar is shown below.
...
We now define what it means for a program in $\Lambda^\to$ to be well-typed. The following typing judgments assign a type to a $\Lambda^\to$ program.

\begin{singlespace}
  \begin{mathpar}
    \Infer{$\Lambda^\to$-TConst}{}{\Gamma\vdash c:b}
    \and
    \Infer{$\Lambda^\to$-TVar}{}{\Gamma,x:\tau\vdash x:\tau}
    \and
    \Infer{$\Lambda^\to$-TLam}{
      \Gamma,x:\tau_1\vdash e:\tau_2
    ...
    \and
    \Infer{$\Lambda^\to$-TFix}{}{\Gamma\vdash(\fix f:\tau.e):\tau}
  \end{mathpar}
\end{singlespace}

The dynamic semantics are not much different than $\Lambda$. Additional evaluation rules are defined for constants and fixpoints; evaluation of $\lambda$-abstractions and function application remains the same.

\begin{singlespace}
  \begin{mathpar}
    \Infer{$\Lambda^\to$-EConst}{}{c\Downarrow c}
    \and
    \Infer{$\Lambda^\to$-ELam}{}{\lambda x:\tau.e \Downarrow \lambda x:\tau.e}
    \and
    \Infer{$\Lambda^\to$-EAp}{
      e_1\Downarrow\lambda x:\tau.e
    }{e_1\ e_2\Downarrow [e_2/x]e}
    \and
    \Infer{$\Lambda^\to$-EFix}{}{\fix f:\tau.e \Downarrow[\fix f:\tau.e/f]e}
    \and
  \end{mathpar}
\end{singlespace}

We may characterize type systems by establishing certain desirable properties. One such property is \textit{soundness}. Soundness means that if a program in $\Lambda^\to$ type-checks, then it will not fail with a type error at run-time. This property is not necessary to prove for $\Lambda$ because there is only one type in $\Lambda$, the recursive type $\tau::=\tau\to\tau$.

There is an additional expression form in $\Lambda^\to$. This is the \textit{fixpoint form}, $\fix f:\tau.e$. The fixpoint is a primitive operator with the same purpose and evaluation behavior as the Y-combinator: it allows for self-reference, and thus general recursion. The reason for the explicit fixpoint operator is that the Y-combinator is ill-typed. Self-reference is inherently poorly-typed and requires a primitive operator, since it involves a function which takes itself as a parameter (leading to an infinitely-recursive arrow type). With the $\fix$ operator, we may express the factorial function as shown below. In this example, we assume that the base type $b\equiv\text{int}$, and that conditionals and primitive integer operations extend $\Lambda^\to$.
...
\todo{write this section, reference Siek06gradualtyping,siek2015refined}
